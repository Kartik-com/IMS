...
async function loadInventory() {
  try {
    const items = await ipcRenderer.invoke("purchase:getItems");
    fullInventory = items;
...
function filterInventory() {
  const searchTerm = document
    ...
    .value.trim()
    .toLowerCase();
  const filteredInventory = fullInventory.filter(
    (item) =>
...
async function addProduct() {
  ...
  try {
    const exists = await ipcRenderer.invoke("purchase:checkBarcode", barcode);
    if (exists) {
  ...
  try {
    await ipcRenderer.invoke("purchase:addItem", product);
    resetForm();
    await loadInventory();
  } catch (error) {
...
async function updateProduct() {
  ...
  const barcodeError = document.getElementById("barcode-error");
  try {
    const existingItem = await ipcRenderer.invoke(
      "purchase:checkBarcode",
      barcode
    );
  ...
  try {
    await ipcRenderer.invoke("purchase:updateItem", product);
    resetForm();
    await loadInventory();
  } catch (error) {
...
async function deleteProduct(id) {
  if (confirm("Are you sure you want to delete this product?")) {
    try {
      await ipcRenderer.invoke("purchase:deleteItem", id);
      resetForm();
      await loadInventory();
    } catch (error) {
